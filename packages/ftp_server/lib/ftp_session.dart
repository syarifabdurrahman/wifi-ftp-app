...
class FtpSession {
  ...
  Future<void> storeFile(String filename) async {
    ...
    try {
      ...
      await fileSink.addStream(dataSocket!);

      await fileSink.close();
      transferInProgress = false;
      await _closeDataSocket();
      sendResponse('226 Transfer complete');
      logger.generalLog('File transfer complete: $filename to $fullPath');
    } catch (e) {
  ...
  ///
  /// Known client-side issue: FileZilla (GnuTLS) may report
  /// "TLS connection was non-properly terminated" after successful
  /// data transfers. The close_notify IS sent by Dart's SecureSocket,
  /// but GnuTLS sometimes fails to process it before the TCP FIN
  /// arrives. The data transfer completes successfully regardless.
  ...
  Future<void> makeDirectory(String dirname) async {
    ...
    try {
      ...
      // Resolve the path directly instead of changing into the new dir,
      // which can fail on some platforms (e.g. Android scoped storage)
      // if createDirectory "succeeds" but the directory isn't immediately
      // visible via existsSync().
      final resolved = fileOperations.resolvePath(dirname);
      final relPath = p.relative(resolved, from: fileOperations.rootDirectory);
      sendResponse('257 "/$relPath" created');
    } catch (e) {
